const ClientAdapter = require('./adapter/clientAdapter');
...
/**
  ...
  */
  ...
  async upsert(id, payload, expiresIn) {
    const result = await this.adapter.upsert(id, payload, expiresIn);
    return result;
  }
   ...
   */
  async find(id) {
    const result = await this.adapter.find(id);
    return result;
  }
   ...
   */
  async consume(id) {
    const result = await this.adapter.consume(id);
    return result;
  }
   ...
   *
   */
    const result = await this.adapter.destroy(id);
    return result;
  }
}
